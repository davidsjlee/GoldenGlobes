...
def extract(data):
    # Code goes here
    winner_pattern = '\swins\s|\swon\s|est\sperformance|est\smovie\s|est\sMovie|est\spicture|est\sPicture'
    ...
    filtered_tweets = []
    for tweet in tweets:
        if re.search(winner_pattern, tweet):
                ...
                if 'television' in award:
                    ...
                    for n in generic_names:
                        if 'Best' not in n and 'Comedy' not in n:
                            award_winners.append(n) 
                elif 'screenplay' in award or 'original' in award:
                    #look in movies file
                    award_winners.append(get_movie(tweet))
                elif 'upporting' in award or 'erformance' in award or 'ctor' in award or 'ctress' in award:
                    people_list = get_people(tweet)
                    for p in people_list:
                        award_winners.append(p)
                else:
                    #look in movies file
                    award_winners.append(get_movie(tweet))
        deduped_winners = {i:award_winners.count(i) for i in award_winners}
        #print deduped_winners
        max = 0
        maxkey = ''
        for key,value in deduped_winners.items():
